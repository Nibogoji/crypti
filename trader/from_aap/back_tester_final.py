...
from models.filter_modeling_v2 import *
from collections import deque
from sklearn.preprocessing import StandardScaler
import os
import random
import math
from src.visualization.plots import TradingGraph
import matplotlib
...
class CustomEnv:
    ...
    def step(self, action_5,plot_flag_5,action_10, plot_flag_10, tax = 0.001):
        ...
        for i, (action, horizon) in enumerate(zip(actions, horizons)):
            if action == 0:  # Hold
                ...
                self.current_position_5 = 0
                self.position_open_pric_5 = current_price
    ...
    def strategy(self, position = 'long'):
        ...
        if self.current_step >= self.training_size:
            ...
            if self.current_step in self.retraining_points or self.time_out > 15:
        ...
        if self.holding_5 == True:
            ...
            sltp_action_5, sltp_flag_5 = self.stop_l_take_p(self.current_position_5, self.position_open_price_5,
                                                            risk_factor_sell=2, risk_factor_buy=1.5)
        ...
        if self.holding_10 == True:
            ...
            sltp_action_10, sltp_flag_10 = self.stop_l_take_p(self.current_position_10, self.position_open_price_10,
                                                              risk_factor_sell=1.8, risk_factor_buy=1.8)
        ...
        if self.holding_period_5 <= 0:
            ...
            if self.current_position_5 == 1:
                if latest_predictions_5.count(0) >= 17: # if predictions are still on buy side prolong holding by 1
                    self.holding_5 = True
                    self.holding_period_5 = 1
                    action_5 = 0
                else:
                ...
                if latest_predictions_5.count(1) >= 17: # if predictions are still on sell side prolong holding by 1
                    self.holding_5 == True
                    self.holding_period_5 = 1
                    action_5 = 0
                else:
        ...
        if self.holding_period_10 <= 0:
            ...
            if self.current_position_10 == 1:
                if latest_predictions_10.count(0) >= 7: # if predictions are still on buy side prolong holding by 2
                    self.holding_10 = True
                ...
                if latest_predictions_10.count(0) >= 7: # if predictions are still on sell side prolong holding by 2
                    self.holding_10 == True
                    self.holding_period_10 = 2
                    action_10 = 0
                else:
...
ohlc = pd.read_csv('data\\external\\refactored_ohlc_5.csv', index_col=0)
aap_data_5 = pd.read_csv('data\\external\\refactored_data_5.csv', index_col='DATE_TRADE_OPEN_POS')
aap_data_10 = pd.read_csv('data\\external\\refactored_data_10.csv', index_col='DATE_TRADE_OPEN_POS')

# aap_data_5_new = pd.read_csv('data\\external\\last_data_5.csv', index_col='DATE_TRADE_OPEN_POS')
...
test_env = CustomEnv(ohlc, aap_data_5 , aap_data_10, initial_balance=1000000, base_bet=1000000,retraining=20, training_size= training_size,lookback_window_size=lookback_window_size, prediction_horizon=(5,10))
