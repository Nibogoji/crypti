...
class CustomEnv:
    ...
    def __init__(self, df, aap_data5,aap_data10,
        ...
        #voting related
        self.best_of_xgb_prediction_5 = deque(maxlen=3)  ## Perche 3 e non 5??##############################################################
    ...
    def get_trained_environment(self, ):

        latest_historical_data_5 = self.aap_data5.iloc[-480-self.prediction_horizon_5:,:]
        latest_historical_data_5.iloc[-self.prediction_horizon_5:,-1] = np.nan

        latest_historical_data_10 = self.aap_data5.iloc[-480-self.prediction_horizon_10:,:]
        latest_historical_data_10.iloc[-self.prediction_horizon_10:,-1] = np.nan


        filter_trained_parameters = {'mean_shift_train':self.mean_shift_train,
                                     'std_shift_train': self.std_shift_train,
                                     'current_xgb_val_score':self.current_xgb_val_score,
                                     'train_test_distances': self.train_test_distances,
                                     'filter_baseline': self.filter_baseline,
                                     'back_to_normal': self.back_to_normal,
                                     'step_to_normal': self.step_to_normal,
                                     'free_will': self.free_will,
                                     'train_test_distances': self.train_test_distances,
                                     'most_important_indexes': self.most_important_indexes
                                     }

        voting_trained_parameters = {'best_of_xgb_prediction_5' : self.best_of_xgb_prediction_5,
                                    'best_of_mlp_prediction_5' : self.best_of_mlp_prediction_5,
                                    'best_of_xgb_prediction_10' : self.best_of_xgb_prediction_10,
                                    'best_of_mlp_prediction_10' : self.best_of_mlp_prediction_10
                                     }

        model_trained_parameters = {'current_xgb_model_5' : self.current_xgb_model_5,
                                    'current_xgb_model_10' : self.current_xgb_model_10,
                                    'current_mlp_model_5' : self.current_mlp_model_5,
                                    'current_mlp_model_10' : self.current_mlp_model_10,
                                    'current_scaler' : self.current_scaler,
                                    'current_training_pca_space' : self.current_training_pca_space,
                                    'current_pcs_receipt' : self.current_pcs_receipt
                                    }

        var_trained_parameters = {'returns' : self.returns,
                                  'step_var' : self.step_var,
                                  'var_history' : self.var_history,
                                  'past_200_close' : self.df[['Close']].iloc[-200:,:],
                                  'volatility': self.volatility,
                                  'orders_history' : self.orders_history,
                                  'market_hitory' : self.market_history,
                                  'hystorical_data_5': latest_historical_data_5,
                                  'hystorical_data_10': latest_historical_data_10
                                  }

        parameters_trained = {'model_trained_parameters' : model_trained_parameters,
                              'filter_trained_parameters' : filter_trained_parameters,
                              'voting_trained_parameters' : voting_trained_parameters,
                              'var_trained_parameters' : var_trained_parameters
                              }
    ...
    def retraining(self, data5, data10, n_syn_features = 2, limit_training = True, reference_points = 10, time_out = False):
        ...
        if self.current_step > 480 and limit_training is True:

            training_data5 = data5.iloc[self.current_step - 480:self.current_step ]
            training_data10 = data10.iloc[self.current_step - 480:self.current_step ]
...
def Play_env(env, visualize, start_render = 0, train_episodes=1, training_batch_size=500):
    ...
    for episode in range(train_episodes):
        ...
        while True:
            ...
            if env.current_step == env.end_step:
                ...
                final_state = env.get_trained_environment()
                break
    ...
    return trades_list, experiment_worths_evo, experiment_returns_evo, vola_ds, vola_ps, final_state



training_size = 12*20 # 12*4
...
aap_data_10 =aap_data_10.iloc[: 1000,:]
aap_data_5 =aap_data_5.iloc[: 1000,:]
ohlc =ohlc.iloc[: 1000,:]


train_env = CustomEnv(ohlc, aap_data_5, aap_data_10, Render_range=500, initial_balance=2000, base_bet=1500, retraining=20 * 48, training_size= training_size, lookback_window_size=lookback_window_size, prediction_horizon=(10, 80))

trades_list, worth_evolution, returns, vola_ds, vola_ps, final_state = Play_env(train_env, start_render = training_size, visualize=False, train_episodes=1)

import pickle
parameters_file = open("trained_parameters.pkl", "wb")
pickle.dump(final_state, parameters_file)
