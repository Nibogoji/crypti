...
class TradingGraph:
    ...
    def __init__(self, Render_range):
        ...
        # Formatting Date
        self.date_format = mpl_dates.DateFormatter('%d-%m-%Y %H:%M:%S')
    ...
    def render(self, Date, Open, High, Low, Close, Volume, net_worth, trades, indicator1, indicator2,pure_investment):
        # append volume and net_worth to deque list
        self.indicator1.append(indicator1)
        self.indicator2.append(indicator2)
        ...
        self.ax1.clear()
        candlestick_ohlc(self.ax1, self.render_data, colorup='green', colordown='red',width=0.8/480)

        # Put all dates to one list and fill ax2 sublot with volume
        Date_Render_range = [i[0] for i in self.render_data]
        self.ax2.clear()
        self.ax2.plot(Date_Render_range,np.ones((len(Date_Render_range), 1)) * 85)
        self.ax2.plot(Date_Render_range,np.ones((len(Date_Render_range), 1)) * 10)
        self.ax2.plot(Date_Render_range,self.indicator1, label = 'rsi')
        self.ax2.plot(Date_Render_range,self.indicator2,label = 'stoch k')
        self.ax2.legend()
        ...
        self.ax3.plot(Date_Render_range, self.pure_investment, color="purple")
