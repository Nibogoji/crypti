...
class CustomEnv:
    ...
    def __init__(self, df, aap_data5,aap_data10,
        ...
        #voting related
        self.best_of_xgb_prediction_5 = 0
        self.best_of_mlp_prediction_5 = 0
        self.best_of_xgb_prediction_10 = 0
        self.best_of_mlp_prediction_10 = 0
    ...
    def step(self, action_5,plot_flag_5,action_10, plot_flag_10, tax = 0.001):
        ...
        for i, (action, horizon) in enumerate(zip(actions, horizons)):
            if action == 0:  # Hold
                ...
                self.trades.append({'Date': Date,
                                    ...
                                    'type': "buy",
                                    'flag' : action_flag[i],
                                'Net position': (self.quantity_held_5 + self.quantity_held_10 - self.short_held_5 - self.short_held_10)})
        ...
        self.net_position.append({'Date': Date,
                                'Net position': (self.quantity_held_10 + self.quantity_held_5 - self.short_held_5 - self.short_held_10)})
    ...
    def render(self, visualize=False):
        ...
        if visualize:
            ...
            a = self.train_test_distances
            b = self.filter_baseline
    ...
    def retraining(self, data5, data10, n_syn_features = 2, limit_training = True, reference_points = 10, time_out = False):
        ...
        if self.current_step > 480 + self.prediction_horizon_10 and limit_training is True:

            training_data5 = data5.iloc[self.current_step - 480 - self.prediction_horizon_5 : self.current_step - self.prediction_horizon_5 ]
            training_data10 = data10.iloc[self.current_step - 480 - self.prediction_horizon_10:self.current_step - self.prediction_horizon_10]
        ...
        if time_out:
            ...
            training_data5 = data5.iloc[self.current_step - 20 - self.prediction_horizon_5 : self.current_step - self.prediction_horizon_5]
            training_data10 = data10.iloc[self.current_step - 20 - self.prediction_horizon_10: self.current_step - self.prediction_horizon_10]
            self.time_out = 0
            self.free_will = True

        train_Z5, train_features_5, train_target5 = make_categorical_target(training_data5, plot=False)
        train_Z10, train_features_10, train_target10 = make_categorical_target(training_data10, plot=False)
        for c in range(train_features_5.shape[1]):
            if np.array_equal(train_features_5.iloc[:, c].values.reshape(1, -1), train_target5.reshape(1, -1),
                ...
                break
        for c in range(train_features_10.shape[1]):
            if np.array_equal(train_features_10.iloc[:, c].values.reshape(1, -1), train_target10.reshape(1, -1),
                              equal_nan=False):
                print('Sanity check failed : Target in features')
                break
        ...
        scaled_train_features_10 = scaler.fit_transform(train_features_10)
        scaled_train_features_5 = scaler.transform(train_features_5)


        X_train_reduced_5, var_vals_5, pc_receipt_5, pcs_space_5, res_5, pcs_importance_5 = custom_pca(scaled_train_features_5, n_pcs=n_syn_features)
        X_train_reduced_10, var_vals_10, pc_receipt_10, pcs_space_10, res_10, pcs_importance_10 = custom_pca(scaled_train_features_10, n_pcs=n_syn_features)
        # What new pcs are made of
        pc_receipt = pd.DataFrame(pc_receipt_5, columns=['Pc {}'.format(i) for i in range(X_train_reduced_5.shape[1])])
        ...
        importance = []
        for _ in range(int(8/pcs_space_5.shape[0])):
            imp = [i for i in pc_receipt.max(axis=0)]
            importance += list(imp*pcs_importance_5)
            ...
            pc_receipt = pc_receipt.drop(most_important_indexes[-pcs_space_5.shape[0]:],axis = 0)
        ...
        reduced_train_dataset5 = np.concatenate((X_train_reduced_5, categorical_train_target5), axis=1)
        reduced_train_dataset10 = np.concatenate((X_train_reduced_10, categorical_train_target10), axis=1)
        ...
        self.mean_shift_train = np.mean(res_5[-reference_points:])
        self.std_shift_train = np.std(res_5[-reference_points:])
        ...
        self.current_training_pca_space = pcs_space_5
    ...
    def filter(self, scaled_test_features,test_projection, pcs, span = 3, toll = 0.8):
        ...
        self.train_test_distances.append(np.mean( abs(np.asarray(self.mean_shift_train) - np.asarray(live_test_res))))
        ...
        if self.filter_baseline[-1] < w_ttd[-1]:
            flag = 'Not Reliable'
        else:
            flag = 'Reliable'
    ...
    def prediction_pipeline(self, nsf):
        ...
        test_data_5 = test_data.iloc[self.current_step- self.prediction_horizon_5: self.current_step]
        test_data_10 = test_data.iloc[self.current_step - self.prediction_horizon_10: self.current_step]

        scaled_test_features_5 = self.current_scaler.transform(test_data_5.to_numpy())
        scaled_test_features_10 = self.current_scaler.transform(test_data_10.to_numpy())

        test_projection_5 = np.dot(self.current_training_pca_space, scaled_test_features_5.transpose())
        test_reduced_5 = test_projection_5.transpose()

        test_projection_10 = np.dot(self.current_training_pca_space, scaled_test_features_10.transpose())
        test_reduced_10 = test_projection_10.transpose()

        # fiter
        filter_response = self.filter(scaled_test_features_5, test_projection_5,nsf, toll= 0.4)

        xgb_live_predictions_5 = self.current_xgb_model_5.predict(test_reduced_5)
        mlp_live_predictions_5 = self.current_mlp_model_5.predict(test_reduced_5)

        xgb_live_predictions_10 = self.current_xgb_model_10.predict(test_reduced_10)
        mlp_live_predictions_10 = self.current_mlp_model_10.predict(test_reduced_10)

        return list(xgb_live_predictions_5), list(mlp_live_predictions_5), \
               list(xgb_live_predictions_10),list(mlp_live_predictions_10),\
               filter_response
    ...
    def strategy(self, position = 'long'):
        ...
        if self.current_step >= self.training_size + self.prediction_horizon_10:
            ...
            if self.current_step in self.retraining_points or self.time_out > 20 or relative_latest_worths < -0.03*self.orders_history[-5][1]:

                print( self.aap_data10.shape)
            ...
            if self.free_will:
                ...
                if xgb_live_predictions_5.count(1) > xgb_live_predictions_5.count(0):
                    free_action_5 = 1
                    free_plot_flag_5 = 'Opening Position'

                elif xgb_live_predictions_5.count(1) < xgb_live_predictions_5.count(0):
                    free_action_5 = 3
                ...
                if xgb_live_predictions_10.count(1) > xgb_live_predictions_10.count(0) == 1:
                    free_action_10 = 1
                    free_plot_flag_10 = 'Opening Position'

                elif xgb_live_predictions_10.count(1) < xgb_live_predictions_10.count(0):
                    free_action_10 = 3
            ...
            self.best_of_xgb_prediction_5 = xgb_live_predictions_5
            self.best_of_xgb_prediction_10 = xgb_live_predictions_10
            self.best_of_mlp_prediction_5 = mlp_live_predictions_5
            self.best_of_mlp_prediction_10 = mlp_live_predictions_10


            latest_predictions_5 = self.best_of_xgb_prediction_5 + self.best_of_mlp_prediction_5 \
                                   +  self.best_of_xgb_prediction_10[:self.prediction_horizon_5] + self.best_of_mlp_prediction_10[:self.prediction_horizon_5]
            latest_predictions_10 = self.best_of_xgb_prediction_10 + self.best_of_mlp_prediction_10

            relative_acceptane = [0.75, 0.8]


            acceptance_5 = int(len(latest_predictions_5)*relative_acceptane[1]) if self.volatility/self.df['Open'].iloc[self.current_step] < 0.1 else int(len(latest_predictions_5)*relative_acceptane[0])
            acceptance_10 = int(len(latest_predictions_10)*relative_acceptane[1]) if self.volatility/self.df['Open'].iloc[self.current_step] < 0.1 else int(len(latest_predictions_10)*relative_acceptane[0])
...
def Play_env(env, visualize, start_render =565, train_episodes=1, training_batch_size=500):
    average_net_worth = 0
...
test_env = CustomEnv(ohlc, aap_data_5 , aap_data_10, Render_range=500,initial_balance=2000, base_bet=1500,retraining=20*12, training_size= training_size,lookback_window_size=lookback_window_size, prediction_horizon=(10,20))

trades_list, worth_evolution, returns, vola_ds, vola_ps = Play_env(test_env, start_render = training_size, visualize=False, train_episodes=1)
